...
import type { RuleWithConfidence } from './types';
...
function stripSpecialCharacters(string: string) {
...
function executeFPGrowth(
  ...
  { minSupport, minConfidence }: { minSupport: number, minConfidence: number }
): void {
...
function parse(args: ({ transactionsFile: string, itemNamesFile: string, minSupport: number, minConfidence: number })): void {
  echoProgress(`Reading item names from ${args.itemNamesFile}...`);

  const { minSupport, minConfidence } = args;

  readItemNames(args.itemNamesFile)
    .then(itemNames => {
      ...
      readDataFileByLine(args.transactionsFile)
        .on(LINE_EVENT, line => {
